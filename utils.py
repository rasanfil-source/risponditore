...
import config
from zoneinfo import ZoneInfo  # Aggiungi questa importazione"""
Utility functions for date calculations, filters, and text processing
"""
...
def should_ignore_email(subject: str, content: str, sender_email: str,
    ...
    for keyword in ignore_keywords:
        if keyword.lower() in text:
            print(f"Email ignored due to keyword: '{keyword}'")
            return True
    
    for domain in ignore_senders:
        if domain.lower() in sender_email.lower():
            print(f"Email ignored due to domain: '{domain}'")
            return True
...
def should_ignore_email(subject: str, content: str, sender_email: str,
                        ignore_keywords: List[str], ignore_domains: List[str]) -> bool:
    """
    Check if email should be ignored
    """
    text = (subject + ' ' + content).lower()
